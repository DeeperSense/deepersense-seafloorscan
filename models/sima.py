...
class SimABlock(nn.Module):
    ...
    Args:
        dim (int): Number of input channels.
        patches_resolution (int): Input resulotion.
        num_heads (int): Number of attention heads.
...
class BasicEncoderLayer(nn.Module):
    ...
    Args:
        ...
        norm_layer (nn.Module, optional): Normalization layer. Default: nn.LayerNorm
        downsample (nn.Module | None, optional): Downsample layer at the begining of the layer. Default: None
    """
...
class BasicDecoderLayer(nn.Module):
    ...
    def __init__(self, dim, patches_resolution, depth, num_heads, mlp_ratio=4., qkv_bias=True,
        ...
        assert skip_type in ('add','cat'), "Invalid skip type!"
...
class SimAEncoder(nn.Module):
    ...
    def __init__(self, img_size=256, in_chans=1, num_classes=0, scales=[1.0], patch_size=4,
                 embed_dim=64, depths=[2,2,6,2], num_heads=[2,4,8,16], mlp_ratio=4.,
        ...
        # MULTISCALE Training Mods
        img_size = numpy.array([int(img_size*s) for s in scales])
...
class SimADecoder(nn.Module):
    ...
    def __init__(self, img_size=256, in_chans=1, num_classes=0, scales=[1.0], patch_size=4,
                 embed_dim=64, depths=[6, 2, 2], num_heads=[8, 4, 2], mlp_ratio=4.,
                 qkv_bias=True, drop_rate=0., drop_path_rate=0.1, use_ghost_ffn=False,
                 skip_type='cat', expand_first=False, norm_layer=nn.LayerNorm):
